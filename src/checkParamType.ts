...
function checkParamType(itemName: string, item: any, type: paramTypes | paramTypes[]) {
    const itemType = typeof item;
    if (Array.isArray(type)) { //match on the the array item
        let matched = false;
        for (let t of type) {
            if (t === "array" || t === "Array") {
                if (Array.isArray(item)) matched = true;
            }
            else if (itemType === t) matched = true;
        }

        if (!matched) throw new TypeError(itemName + " must be one of " + type.join(", ") + ", received " + itemType)
    }
    else if ((type === "array" || type === "Array") && Array.isArray(item)) return true;
    else if (itemType !== type) throw new TypeError(itemName + " must be an " + type + ", received " + itemType)
